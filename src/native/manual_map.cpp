...
void die(const std::string& msg) { DWORD code = GetLastError(); std::cerr << "[FATAL] " << msg << " (code=" << code << ")" << std::endl; exit(1); }
void info(const std::string& msg) { std::cout << "[+] " << msg << std::endl; }
void warn(const std::string& msg) { std::cout << "[!] " << msg << std::endl; }

std::string hex_str(uintptr_t v) { char buf[32]; snprintf(buf, sizeof(buf), "0x%llx", (unsigned long long)v); return std::string(buf); }
...
uintptr_t remote_module_base(DWORD pid, const std::string& name_lower) {
    ...
    do {
        std::string mod_name;
        for (int i = 0; e.szModule[i]; i++) { char c = (char)(e.szModule[i] < 128 ? e.szModule[i] : '?'); mod_name.push_back((char)tolower(c)); }
...
const char* resolve_api_set(const char* name) {
    if (strstr(name, "api-ms-win-crt-") || strstr(name, "api-ms-win-core-")) return "ucrtbase.dll";
    return name;
}
...
void dump_sections(const PeData& pe) {
    ...
    for (int i = 0; i < pe.nt->FileHeader.NumberOfSections; i++) {
        auto& s = pe.sections[i]; char name[9] = {0}; memcpy(name, s.Name, 8);
        info("  [" + std::to_string(i) + "] \"" + std::string(name) + "\" VA=" + hex_str(s.VirtualAddress) + " raw=" + std::to_string(s.SizeOfRawData));
    }
}
...
bool setup_tls(HANDLE p, uintptr_t dll_base, PeData& pe) {
    ...
    if (tls_dir.Size == 0 || tls_dir.VirtualAddress == 0) { info("TLS: none"); return true; }

    uintptr_t tls_dir_addr = dll_base + tls_dir.VirtualAddress;
    IMAGE_TLS_DIRECTORY64 tls; SIZE_T rd;
    if (!ReadProcessMemory(p, (LPCVOID)tls_dir_addr, &tls, sizeof(tls), &rd)) { warn("TLS: read failed"); return false; }
    if (tls.StartAddressOfRawData == 0) { info("TLS: empty"); return true; }

    size_t data_sz = (size_t)(tls.EndAddressOfRawData - tls.StartAddressOfRawData);
    size_t zero_sz = tls.SizeOfZeroFill;
    size_t total = data_sz + zero_sz;

    info("TLS: data=" + std::to_string(data_sz) + " zero=" + std::to_string(zero_sz) + " total=" + std::to_string(total));

    LPVOID block = VirtualAllocEx(p, nullptr, total, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    if (!block) { warn("TLS: alloc failed"); return false; }
    uintptr_t bp = (uintptr_t)block;

    if (data_sz > 0) {
        std::vector<uint8_t> buf(data_sz);
        uintptr_t src = dll_base + (tls.StartAddressOfRawData - pe.nt->OptionalHeader.ImageBase);
        if (ReadProcessMemory(p, (LPCVOID)src, buf.data(), data_sz, &rd)) {
            SIZE_T wr; WriteProcessMemory(p, block, buf.data(), data_sz, &wr);
        }
    }
    if (zero_sz > 0) { std::vector<uint8_t> z(zero_sz, 0); SIZE_T wr; WriteProcessMemory(p, (LPVOID)(bp + data_sz), z.data(), zero_sz, &wr); }

    IMAGE_TLS_DIRECTORY64 patched = tls;
    patched.StartAddressOfRawData = bp;
    patched.EndAddressOfRawData = bp + total;
    patched.SizeOfZeroFill = 0;
    SIZE_T wr; WriteProcessMemory(p, (LPVOID)tls_dir_addr, &patched, sizeof(patched), &wr);

    info("TLS: block @ " + hex_str(bp) + " (" + std::to_string(total) + " bytes)");
    return true;
}
...
bool resolve_imports(HANDLE p, DWORD pid, uintptr_t base, PeData& pe) {
    ...
    if (dir.Size == 0) { info("No imports (fully static)"); return true; }
    ...
    while (desc->Name != 0) {
        ...
        while (og->u1.AddressOfData != 0) {
            ...
            if (!lf) ur++;
            SIZE_T wr;
            uintptr_t addr = base + (desc->FirstThunk + (uintptr_t)tk - (uintptr_t)(IMAGE_THUNK_DATA64*)(pe.raw.data() + rva_to_ptr(desc->FirstThunk, pe)));
            WriteProcessMemory(p, (LPVOID)addr, &rf, sizeof(rf), &wr);
            og++; tk++;
        }
        FreeLibrary(hm);
...
bool inject_via_apc(HANDLE p, DWORD pid, uintptr_t dll_base, uintptr_t entry_rva) {
    ...
    SIZE_T wr; WriteProcessMemory(p, sc_mem, sc.data(), sc.size(), &wr);
    ...
    bool queued = false; int total = 0;
    do {
        if (te.th32OwnerProcessID != pid) continue;
        total++; if (queued) continue;
        HANDLE h = OpenThread(THREAD_SET_CONTEXT, FALSE, te.th32ThreadID);
        if (!h) continue;
        if (QueueUserAPC((PAPCFUNC)sc_mem, h, (ULONG_PTR)dll_base)) { info("APC -> TID=" + std::to_string(te.th32ThreadID)); queued = true; }
        CloseHandle(h);
    } while (Thread32Next(s, &te));
    CloseHandle(s);

    info(std::to_string(queued) + " APC / " + std::to_string(total) + " threads");
...
bool is_x64_process(DWORD pid) {
    ...
    if (!h) return false;
    BOOL w = FALSE; IsWow64Process(h, &w); CloseHandle(h); return !w;
}

int wmain(int argc, wchar_t* argv[]) {
    if (argc < 3) { std::wcout << L"Manual Map Injector (APC + TLS)\n" << L"Usage: manual_map.exe <dll> --process <name>\n"; return 1; }
